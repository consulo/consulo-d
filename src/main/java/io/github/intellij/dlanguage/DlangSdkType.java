...
public class DlangSdkType extends SdkType
{
	...
	static
	{
		...
		{
			LOG.warn(String.format("We didn't cater for %s", SystemInfo.OS_NAME + ":" + SystemInfo.OS_VERSION));
			DEFAULT_DMD_PATHS = new File[]{};
			DEFAULT_DOCUMENTATION_PATHS = new File[]{};
			DEFAULT_PHOBOS_PATHS = new File[]{};
			DEFAULT_DRUNTIME_PATHS = new File[]{};
	...
	{
		return SdkType.EP_NAME.findExtension(DlangSdkType.class);
	}
	...
	{
		return Arrays.stream(DEFAULT_DMD_PATHS).map(File::getPath).collect(Collectors.toList());
	}
	...
	private Optional<VirtualFile> firstVirtualFileFrom(File[] files)
	{
		for(File f : files)
		{
			if(f.isDirectory())
			{
				return Optional.ofNullable(LocalFileSystem.getInstance().findFileByPath(f.getAbsolutePath()));
			}
		}
	...
	private SetupStatus setupSDKPathsFromWindowsConfigFile(final Sdk sdk, final SdkModificator sdkModificator)
	{
		...
		try
		{
			...
			final Pattern phobosPattern = Pattern.compile("\"-I%@P%([\\.\\\\A-Za-z]+phobos[\\.\\\\A-Za-z]*)\"");
			final String[] druntime = new String[1];
			final Pattern druntimePattern = Pattern.compile("\"-I%@P%([\\.\\\\A-Za-z]+druntime\\\\import[\\.\\\\A-Za-z]*)\"");
			;
	...
	private String getDmdVersion(final String sdkHome)
	{
		if(isValidSdkHome(sdkHome))
		{
			...
			try
			{
				final ProcessOutput output = new CapturingProcessHandler(cmd.createProcess(), Charset.defaultCharset(), cmd.getCommandLineString()).runProcess();
