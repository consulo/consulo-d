...
public class DlangRunDmdState extends CommandLineState implements ProcessListener
{
	...
	protected ProcessHandler startProcess() throws ExecutionException
	{
		...
		catch(final ExecutionException e)
		{
			...
			{
				Notifications.Bus.notify(new Notification("DMD run configuration", "DMD settings", "DMD executable path is " + (isEmpty ? "empty" : "not specified correctly") + "<br/><a " +
						"href='configure'>Configure</a>", NotificationType.ERROR), config.getProject());
			}
			throw e;
		}
	}
	 ...
	 */
	private GeneralCommandLine getDmdCommandLine(final DlangRunDmdConfiguration config) throws ModuleNotFoundException, NoValidDlangSdkFound, NoSourcesException, ExecutionException
	{
		...
		if(sdk != null && DlangSdkType.class.isAssignableFrom(sdk.getSdkType().getClass()))
		{
			...
			final GeneralCommandLine cmd = new GeneralCommandLine().withWorkDirectory(config.getProject().getBasePath()).withCharset(Charset.defaultCharset()).withExePath(dlangSdkType.getDmdPath
					(sdk)).withParameters(dmdParameters);
