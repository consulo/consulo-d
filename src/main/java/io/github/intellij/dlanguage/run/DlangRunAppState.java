...
public class DlangRunAppState extends CommandLineState
{
	...
	public ProcessHandler startProcess() throws ExecutionException
	{
		...
		catch(final ExecutionException e)
		{
			...
			{
				Notifications.Bus.notify(new Notification("D App run configuration", "D App settings", "D application executable path is " + (isEmpty ? "empty" : "not specified correctly") +
						"<br/><a href='configure'>Configure</a> output folder", NotificationType.ERROR), config.getProject());
			}
			throw e;
		}
	}
	 ...
	 **/
	public GeneralCommandLine getExecutableCommandLine(final DlangRunAppConfiguration config) throws ModuleNotFoundException, NoValidDlangSdkFound
	{
